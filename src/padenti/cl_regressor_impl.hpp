...
// TODO: confine kernel source vector declaration within #ifdef clause to
...
template <typename ImgType, unsigned int nChannels, typename FeatType, unsigned int FeatDim,
	  unsigned int RDim>
CLRegressor<ImgType, nChannels, FeatType, FeatDim, RDim>::CLRegressor(
...
template <typename ImgType, unsigned int nChannels, typename FeatType, unsigned int FeatDim,
	  unsigned int RDim>
CLRegressor<ImgType, nChannels, FeatType, FeatDim, RDim>&
CLRegressor<ImgType, nChannels, FeatType, FeatDim, RDim>::operator<<(
  const RTree<FeatType, FeatDim, RDim>& tree)
{
  ...
  m_clTreeValuesBuff.push_back(cl::Buffer(m_clContext,
					  CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,
...
template <typename ImgType, unsigned int nChannels, typename FeatType, unsigned int FeatDim,
	  unsigned int RDim>
void CLRegressor<ImgType, nChannels, FeatType, FeatDim, RDim>::predict(
  ...
  Image<unsigned char, 1> &mask)
{
  ...
  m_clPredictKern.setArg(9, m_clTreeValuesBuff.at(treeID));
...
template <typename ImgType, unsigned int nChannels, typename FeatType, unsigned int FeatDim,
	  unsigned int RDim>
void CLRegressor<ImgType, nChannels, FeatType, FeatDim, RDim>::_initImgObjects(size_t width, size_t height, bool deallocate)
{
  if (deallocate)
  {
    m_clQueue.enqueueUnmapMemObject(m_clPredictImgPinn, m_clPredictImgPinnPtr);
